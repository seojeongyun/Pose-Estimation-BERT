...
class Bert(nn.Module):
    ...
    def forward(self, x, segs, mask):
        ...
        if(self.finetune):
            top_vec = self.model.encoder(x, attention_mask=extended_mask, return_dict=False)
        else:
            self.eval()
            with torch.no_grad():
                top_vec = self.model.encoder(x, attention_mask=extended_mask, return_dict=False)
...
class ExtSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint):
        ...
        self.cls_emb = nn.Parameter(torch.empty(hidden_size))
        nn.init.normal_(self.cls_emb, mean=0.0, std=0.02)

        self.to(device)

    def forward(self, src, segs, mask, exercise_targets=None,
                condition_labels=None, condition_mask=None, threshold=0.5):
        top_vec = self.bert(src, segs, mask)
        sent_scores = self.ext_layer(
            top_vec,
            mask,
